...
const outputPath = isDev ? '' : isWuHan ? appConfig.relativeWhPrefix : appConfig.relativePrefix;
function getPublicPath() {
  if (appConfig.prodPublicPath === './') {
    ...
    switch (process.env.PLACE) {
      case 'wuhan':
        return appConfig.absoluteWhPrefix
      default:
...
function getAppPrefix() {
  switch (process.env.PLACE) {
    case 'wuhan':
      return JSON.stringify(appConfig.absoluteWhPrefix)
    default:
      return JSON.stringify(appConfig.absolutePrefix)
  }
}
...
module.exports = {
  ...
  plugins: [
    new webpack.DefinePlugin({
      ...
      'process.env.APP_PREFIX': getAppPrefix(),
      'process.env.PLACE': JSON.stringify(process.env.PLACE),
      'process.env.ISOFFICIAL':process.env.ISOFFICIAL,
    }),
    new HtmlWebpackPlugin({
      template: 'assets/index.html',
      templateParameters: {
        appPrefix: process.env.NODE_ENV == 'development' ? '/' : appConfig.prodPublicPath + outputPath,
