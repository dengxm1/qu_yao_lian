...
const ProductionUnit = (props) => {
  ...
  const getData = (page, pageSize, searchData) => {
    ...
    informFillingApi.supplierList(params).then((res) => {
      if (res?.data?.data) {
  ...
  const submitUpload = () => {
    ...
    if (file) {
      informFillingApi.uploadSupplierModel(formData).then((res) => {
        if (res?.data?.success) {
  ...
  const handleDelete = (record) => {
    informFillingApi.supplierDelete({ companyId: record.companyId }).then((res) => {
      if (res?.data?.data) {
  ...
  const handleDeleteAll = () => {
    ...
    if (selectedRowKeys.length) {
      ...
      informFillingApi.supplierDeleteList(ids).then((res) => {
        if (res?.data?.data) {
