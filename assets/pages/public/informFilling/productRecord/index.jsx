...
const ProductRecord = (props) => {
  ...
  const getData = (page, pageSize, searchData) => {
    ...
    informFillingApi.manufacturerList(params).then((res) => {
      if (res?.data?.data) {
  ...
  const submitUpload = () => {
    ...
    if (file) {
      informFillingApi.uploadManufacturerModel(formData).then((res) => {
        if (res?.data?.success) {
  ...
  const handleDelete = (record) => {
    informFillingApi.manufacturerDelete({ companyId: record.companyId }).then((res) => {
      if (res?.data?.data) {
  ...
  const handleDeleteAll = () => {
    ...
    if (selectedRowKeys.length) {
      ...
      informFillingApi.manufacturerDeleteList(ids).then((res) => {
        if (res?.data?.data) {
