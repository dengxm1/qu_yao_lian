...
const AddProduct = (props) => {
  ...
  const handleSubmit = () => {
    form.validateFields().then((values) => {
      const params = {
        ...values,
        asManufacturer: saveCheck
      };
      if (type === 'add') {
        informFillingApi.manufacturerAdd(params).then((res) => {
          if (res?.data?.data) {
        ...
        informFillingApi.manufacturerUpdate(params).then((res) => {
          if (res?.data?.data) {
    ...
    >
      <div className="modal-first">
        <Form form={form} {...formItemLayout}>
          ...
          <Form.Item name="asManufacturer" label={''} rules={[{ required: false }]}>
            <Checkbox
